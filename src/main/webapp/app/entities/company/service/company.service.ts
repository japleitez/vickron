...
type RestOf<T extends ICompany | NewCompany> = Omit<T, 'start' | 'end'> & {
  start?: string | null;
  end?: string | null;
};
...
export class CompanyService {
  ...
  protected convertDateFromClient<T extends ICompany | NewCompany | PartialUpdateCompany>(company: T): RestOf<T> {
    return {
      ...company,
      start: company.start?.toJSON() ?? null,
      end: company.end?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCompany: RestCompany): ICompany {
    return {
      ...restCompany,
      start: restCompany.start ? dayjs(restCompany.start) : undefined,
      end: restCompany.end ? dayjs(restCompany.end) : undefined,
    };
  }
