...
type RestOf<T extends IPerson | NewPerson> = Omit<T, 'start' | 'end'> & {
  start?: string | null;
  end?: string | null;
};
...
export class PersonService {
  ...
  protected convertDateFromClient<T extends IPerson | NewPerson | PartialUpdatePerson>(person: T): RestOf<T> {
    return {
      ...person,
      start: person.start?.toJSON() ?? null,
      end: person.end?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPerson: RestPerson): IPerson {
    return {
      ...restPerson,
      start: restPerson.start ? dayjs(restPerson.start) : undefined,
      end: restPerson.end ? dayjs(restPerson.end) : undefined,
    };
  }
